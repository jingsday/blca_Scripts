...
setwd('/Users/lidiayung/project/resource/perturbations')

# get data location
dirs <- list.files(path = '/Users/lidiayung/project/resource/perturbations/GSE174182_RAW', recursive = F, full.names = F)
...
ggsave(p1,"featureplot.png",width=10,height=6,dpi=300)
...
clusters_0_4 <- readRDS("/home/jing/resource/murine_blca/scripts/clusters_0_4.rds")
# Visualization
...
p4 <- DimPlot(murine_clean, reduction = 'umap', group.by = 'Sample',
              cols=c('red','green','blue','yellow','brown','orange','purple'))
combined<-p3+p4
ggsave("reso1p.png", plot = combined, width = 10, height = 6, dpi = 300)
murine.markers <- FindAllMarkers(sct_murine, only.pos = TRUE)
...
p3 <- DimPlot(seurat.integrated, reduction = 'umap')
p4 <- DimPlot(seurat.integrated, reduction = 'umap', split.by  = 'Sample',)
combined <-p3+p4
ggsave("splitstrike2.png", plot = combined, width = 10, height = 6, dpi = 300)


p4 <- DimPlot(seurat.integrated, reduction = 'umap', split.by  = 'Sample')

ggsave("splitstrike3.png", plot = p4, width = 30, height = 6, dpi = 300)
